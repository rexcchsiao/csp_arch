  ...
  const budgetOwnerRows = [
    ...
    ...softwareDifferentiation.serviceOfferings.budgetMap.rows.filter((row) => (
      [
        ...
        'AI Retrofit Readiness Assessment',
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-200">
              ...
              <div className="rounded-2xl border border-teal-200 bg-teal-50 p-5 mb-6">
                ...
                <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4 text-sm text-slate-700 mb-4">
                  ...
                  <div className="rounded-xl border border-teal-100 bg-white/90 p-4">
                    <p className="font-black text-slate-900 mb-1">4. FinOps / BU owner：批准的是 shared GPU 成本秩序，而不是 only cost visibility</p>
                    <p className="leading-relaxed">FinOps Foundation 已把成本治理定義成跨 Finance、Engineering、ITAM、ITFM/TBM、Security、ITSM 的協作 operating model。這代表 AI 叢集一旦進入共享模式，真正會被核准的不是 another dashboard，而是 showback / chargeback、quota policy、budget guardrail 與 token / inference 成本責任到底由誰接住。</p>
                  </div>
                </div>
                <div className="rounded-xl border border-teal-100 bg-white p-4">
                  <p className="text-sm font-semibold text-teal-900">董事會該記住的一句話：<span className="text-slate-700">硬體部門通常只打進採購桌；軟體部門一旦成立，Foxconn 就能同時對接上線速度、持續可用性、治理證據與 shared GPU 經濟學這 4 類決策桌，這也是它更像高毛利產品線、而不是附屬支援功能的原因。</span></p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 mb-6">
                ...
                <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-4 text-sm text-slate-700">
                  ...
                  <div className="rounded-xl border border-emerald-100 bg-white/90 p-4">
                    ...
                    <p className="leading-relaxed">客戶最後買單的不是 API、dashboard 或 cost panel，而是誰能把訊號接成 maintenance window、rollback、dispatch readiness、RCA / evidence pack、factory truth / warranty evidence、GPU quota、showback policy 與 SLA / 預算 guardrail。</p>
                  </div>
                  <div className="rounded-xl border border-emerald-100 bg-white/90 p-4">
                    <p className="font-black text-slate-900 mb-1">這正是軟體部門最該佔的位置</p>
                    <p className="leading-relaxed">當底層能力更開放，Foxconn 更應把軟體部門定位成 operating layer owner：把標準化硬體與成本訊號，翻成可治理、可問責、可續約的 Day-2 服務結果，直接對接 SRE、Platform、FinOps、Facilities 與 Compliance 預算，並把 Foxconn 的製造基線變成別人難複製的軟體服務。</p>
                  </div>
                </div>
                <div className="mt-4 rounded-xl border border-emerald-100 bg-white p-4 mb-4">
                  <p className="text-sm font-semibold text-emerald-900">董事會該記住的一句話：<span className="text-slate-700">開放標準沒有削弱軟體部門，反而把價值從「設備可管理 / 成本可視化」推向「營運可問責 / 預算可治理 / 製造證據可續約」；沒有軟體部門，Foxconn 只能提供訊號，有軟體部門，Foxconn 才能把 factory truth 變成可收費、可續約的結果。</span></p>
                </div>
